...
data <- data.frame(Times, as.numeric(as.vector(EPC$Global_active_power)), 
                   as.numeric(as.vector(EPC$Sub_metering_1)), 
                   as.numeric(as.vector(EPC$Sub_metering_2)), 
                   as.numeric(as.vector(EPC$Sub_metering_3)),
                   as.numeric(as.vector(EPC$Voltage)),
                   as.numeric(as.vector(EPC$Global_reactive_power))
                   )
...
with(data, {
        ...
        legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
               lty=c(1,1), lwd=c(0.5,0.5),col =c("black","red","blue"), cex = 0.85)
})
